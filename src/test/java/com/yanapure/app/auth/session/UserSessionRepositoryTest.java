...
public class UserSessionRepositoryTest {
    ...
    void testFindByAccessTokenAndActiveTrue() {
        ...
        UserSession session = new UserSession(userId, accessToken, refreshToken,
                expiresAt, refreshExpiresAt, "192.168.1.1", "Mozilla/5.0");
        entityManager.persistAndFlush(session);

        // When
        Optional<UserSession> found = userSessionRepository
    ...
    void testFindByRefreshToken() {
        ...
        UserSession session = new UserSession(userId, accessToken, refreshToken,
                expiresAt, refreshExpiresAt, "192.168.1.2", "Mozilla/5.0");
        entityManager.persistAndFlush(session);
    ...
    void testFindByUserIdAndActiveTrueOrderByCreatedAtDesc() {
        ...
        UserSession session1 = new UserSession(userId, "token1", "refresh1",
                now.plusSeconds(3600), now.plusSeconds(7200), "192.168.1.1", "Mozilla/5.0");
        session1.setCreatedAt(now.minusSeconds(100));
        entityManager.persistAndFlush(session1);

        UserSession session2 = new UserSession(userId, "token2", "refresh2",
                now.plusSeconds(3600), now.plusSeconds(7200), "192.168.1.2", "Mozilla/5.0");
    ...
    void testFindByExpiresAtBefore() {
        ...
        UserSession expiredSession = new UserSession(UUID.randomUUID(), "token1", "refresh1",
                now.minusSeconds(100), now.plusSeconds(7200), "192.168.1.1", "Mozilla/5.0");
        entityManager.persistAndFlush(expiredSession);

        UserSession activeSession = new UserSession(UUID.randomUUID(), "token2", "refresh2",
                now.plusSeconds(3600), now.plusSeconds(7200), "192.168.1.2", "Mozilla/5.0");
        entityManager.persistAndFlush(activeSession);

        // When
        List<UserSession> expired = userSessionRepository
    ...
    void testCountByUserIdAndActiveTrue() {
        ...
        UserSession activeSession1 = new UserSession(userId, "token1", "refresh1",
                now.plusSeconds(3600), now.plusSeconds(7200), "192.168.1.1", "Mozilla/5.0");
        entityManager.persistAndFlush(activeSession1);

        UserSession activeSession2 = new UserSession(userId, "token2", "refresh2",
                now.plusSeconds(3600), now.plusSeconds(7200), "192.168.1.2", "Mozilla/5.0");
        entityManager.persistAndFlush(activeSession2);

        UserSession inactiveSession = new UserSession(userId, "token3", "refresh3",
                now.plusSeconds(3600), now.plusSeconds(7200), "192.168.1.3", "Mozilla/5.0");
    ...
    void testFindByClientIpAndCreatedAtAfter() {
        ...
        UserSession recentSession = new UserSession(UUID.randomUUID(), "token1", "refresh1",
                now.plusSeconds(3600), now.plusSeconds(7200), clientIp, "Mozilla/5.0");
        recentSession.setCreatedAt(now.minusSeconds(100));
        entityManager.persistAndFlush(recentSession);

        UserSession oldSession = new UserSession(UUID.randomUUID(), "token2", "refresh2",
                now.plusSeconds(3600), now.plusSeconds(7200), clientIp, "Mozilla/5.0");
        ...
        // When
        List<UserSession> sessions = userSessionRepository
                .findByClientIpAndCreatedAtAfter(clientIp, since);
