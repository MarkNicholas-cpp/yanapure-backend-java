...
public class UserControllerTest {
  ...
  void testGetCurrentUserSuccess() throws Exception {
    ...
    // When & Then
    mockMvc
        .perform(get("/api/users/me").header("Authorization", "Bearer valid-token"))
  ...
  void testGetUserByIdAsAdmin() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/users/" + targetUser.getId()).header("Authorization", "Bearer valid-token"))
        .andExpect(status().isOk())
  ...
  void testUpdateUserRoleAsAdmin() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/users/" + targetUser.getId() + "/role")
                .header("Authorization", "Bearer valid-token")
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"role\": \"ADMIN\"}"))
        .andExpect(status().isOk())
  ...
  void testDeleteSelf() throws Exception {
    ...
    mockMvc
        .perform(
            delete("/api/users/" + adminUser.getId()).header("Authorization", "Bearer valid-token"))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.success").value(false))
  ...
  void testGetUserStatsAsAdmin() throws Exception {
    ...
    // When & Then
    mockMvc
        .perform(get("/api/users/stats").header("Authorization", "Bearer valid-token"))
        .andExpect(status().isOk())
