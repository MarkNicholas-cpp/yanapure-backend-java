...
public class AdminControllerTest {
    ...
    void testCreateAdminUserWithApiException() throws Exception {
        ...
        when(adminService.createAdminUser(anyString(), anyString(), anyString()))
                .thenThrow(new ApiException("USER_ALREADY_EXISTS", "User with this phone number already exists"));
    ...
    void testPromoteToAdminSuccess() throws Exception {
        ...
        // When & Then
        mockMvc.perform(put("/api/admin/users/{userId}/promote", promotedUser.getId())
                .header("Authorization", "Bearer valid-token"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
    ...
    void testDemoteFromAdminSuccess() throws Exception {
        ...
        // When & Then
        mockMvc.perform(put("/api/admin/users/{userId}/demote", demotedUser.getId())
                .header("Authorization", "Bearer valid-token"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
